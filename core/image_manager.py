...
def get_image(
    ...
    cache: CacheManager,
    *,
    skip_cache_for_local: bool = True,  # NEW parameter
    brand_color: Tuple[int, int, int] = (60, 60, 90),
    label: str = "",
    fonts_dir: Optional[Path] = None,
    placeholder_size: Tuple[int, int] = (1080, 1920),
    ...
    """
    Resolve `source` to a PIL Image.
    
    If skip_cache_for_local=True and source is a local file, load directly
    without going through the cache (faster for story_prompt_studio workflows
    where images are already local and stable).
    """
    try:
        if _is_url(source):
            # Remote URL - always cache
            suffix = Path(urlparse(source).path).suffix or ".jpg"
            cached_path = cache.image_cache_path(source, suffix=suffix)
            if not cache.is_cached(cached_path):
                logger.info("Downloading image: %s", source)
            ...
            # Skip cache for local files - just load directly
            if skip_cache_for_local:
                logger.debug(f"Loading local image (no cache): {path}")
                return Image.open(path).convert("RGB")
            
            # Original behavior - cache even local files
            cached_path = cache.image_cache_path(str(path.resolve()), suffix=path.suffix)
            if not cache.is_cached(cached_path):
